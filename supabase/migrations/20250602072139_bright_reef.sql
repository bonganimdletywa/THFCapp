...
-- Step 9: Create new RLS policies with simplified checks
-- Basic policy to allow all authenticated users to read their own data
...
-- Allow zoho_admin role to read all users
CREATE POLICY admin_read_all
ON users
FOR SELECT
TO authenticated
USING (
  (SELECT role FROM users WHERE id = auth.uid()) = 'zoho_admin'
);

-- Allow zoho_admin role to insert new users
...
-- Allow zoho_admin role to update any user
